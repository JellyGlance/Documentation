# ![docker](../icons/brands/docker.svg){ .page-brand } Getting Started
...
Download the [example docker-compose.yml](../downloads/docker-compose.yml) into an empty folder, update the passwords and `JWT_SECRET`, then run the following command from that folder.
...
These commands run in the [JellyGlance application repository](https://github.com/Nerdy-Technician/JellyGlance), not this documentation project. Clone that repository first.

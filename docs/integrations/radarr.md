# ![Radarr](../icons/selfhst/radarr.svg){ .page-brand } Connect Radarr

Bring movie release dates, monitored movie context, and integration health into JellyGlance.
...
- Have the API key under Radarr Settings → General → Security.
...
Open **Calendar** and compare an upcoming movie with Radarr. A successful connection can still produce an empty calendar if there are no applicable releases. Confirm the title is configured in Radarr before troubleshooting JellyGlance.

JellyGlance reads service data; configure quality profiles, root folders, indexers, and download clients in Radarr itself. [Servarr documents API keys under General → Security](https://github.com/Servarr/Wiki/blob/master/radarr/settings.md).
